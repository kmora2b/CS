...
public class TrafficController implements Runnable {
	...
	TrafficController(TrafficModel model){
		this.model = model;
	
	}
	
	TrafficController(TrafficGUIView view){
		this.view = view;
		initView();
	...
	class MenuListener implements ActionListener, ItemListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getActionCommand().equalsIgnoreCase("Open")) {
				...
				data.readFile();
				//String xs = data.writeXmlDocumentToXmlFile();
				//data.parseXML(xmlS);
	...
	public void animateOrbit(int vehicleNum) {
		...
		while(i < model.record.size()){
			if (model.record.get(i).get((vehicleNum+1)*2)!=0 && model.record.get(i).get((vehicleNum+1)*2) < 2 * Math.PI){
				//view.vehicleCoords[vehicleNum][0] = (view.trackRX/2 + view.vehicleR/2) * Math.cos(angle) + view.winSizeX/2 - view.vehicleR/2 + Math.cos(model.record.get(i).get((vehicleNum+1)*2));
				//view.vehicleCoords[vehicleNum][1] = (view.trackRY/2 + view.vehicleR/2) * Math.sin(angle) + view.winSizeY/2 - view.vehicleR/2 + Math.sin(model.record.get(i).get((vehicleNum+1)*2));
				view.vehicleCoords[vehicleNum][0] = (view.trackRX/2 + view.vehicleR/2 + (view.vehicleR * (vehicleNum+1) )) * Math.cos(angle) + view.winSizeX/2 - view.vehicleR/2 + Math.cos(model.record.get(i).get((vehicleNum+1)*2));
				view.vehicleCoords[vehicleNum][1] = (view.trackRY/2 + view.vehicleR/2) * Math.sin(angle) + view.winSizeY/2 - view.vehicleR/2 + Math.sin(model.record.get(i).get((vehicleNum+1)*2));
