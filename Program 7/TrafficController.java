...
public class TrafficController{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getActionCommand().equalsIgnoreCase("Open")) {
				...
				if (data.getList() == null){
					...
					ArrayList<Double> segLens = model.convertStringToDoubleList(model.getSegLenList());
					view.initShapes(model.getAllDrivers().size(), model.getSegmentsList().size(), segLens, model.getTotalDistance());
	...
	public void animateOrbit(int vehicleNum) {
		...
		for (double t = 0; t < 2 * Math.PI; t += 0.01) {
			...
			view.vehicleCoords[vehicleNum][0] = (view.trackROut/2 - view.vehicleR/2) * Math.cos(angle) + view.winSizeX/2 - view.vehicleR/2;
			
			view.vehicleCoords[vehicleNum][1] = (view.trackROut/2 - view.vehicleR/2) * Math.sin(angle) + view.winSizeY/2 - view.vehicleR/2;
	...
	public void storeSegmentInfo(Element el){
		...
		System.out.println(segNum + " " + segLen + " "+ segSpeedLim);
		
		model.getSegNumList().add(segNum);
		model.getSegLenList().add(segLen);
		model.getSegSpeedLimList().add(segSpeedLim);
		
		Segment segment = new Segment(Integer.valueOf(segNum), Double.valueOf(segLen), Double.valueOf(segSpeedLim));
				...
				if (el.getNodeName().equalsIgnoreCase("DRIVER")) {
					...
					for (int j = 0; j < model.getAllDriverTypes().size(); j++){
						...
						if(driverDriverType.equalsIgnoreCase(model.getAllDriverTypes().get(j).getDriverTypeName())){
							Driver driver = new Driver(driverName, insertTimeDelay,model.getAllDriverTypes().get(j));
