...
class DriverTypesSim extends CourseSim {
	...
	private CourseSim course;
	private ArrayList<Driver> allDrivers;
	private ArrayList<DriverType> allDriverTypes = new ArrayList<DriverType>();
	private ArrayList<ArrayList<String>> columnNames = new ArrayList<ArrayList<String>>(); 
	private final double maxBrakeRate = 10.2273;
	
	private DriverTypesSim() {
		//Read in file and save its nodelist of the xml file
		ArrayList<ArrayList<Double>> printData = new ArrayList<ArrayList<Double>>();
		XMLFileData data = new XMLFileData("input.xml");
		data.readFile();
	...
	private void setColumnNames(ArrayList<ArrayList<String>> columnNames) {
	...
	public ArrayList<ArrayList<Double>> calculateDistanceValuesForDrivers(double numOfLaps,double totalDistance, double [][] courseInfo){
		...
		while (distanceCount[0] < (numOfLaps * totalDistance) && distanceCount[1] < (numOfLaps *totalDistance) && distanceCount[2] < (numOfLaps *totalDistance)) {
			...
			speedLim = setDriverSpeedLim(currSeg, timeSec, courseInfo, currSegLen);
			...
			for(int i = 0; i < allDrivers.size(); i++){
				for(int j =1 ; j < allDrivers.size()-1; j++){
					double d0 =speedLim[i] * convertSecsToHours(allDrivers.get(i).getDriverType().getFollowTime());
					double d1 = distanceCount[j];
					double dd = distanceCount[i] - d1;
					
					if( d0 < (distanceCount[i] - d1))
	...
	private double[] setDriverSpeedLim(int currSeg[], double timeSec,double [][] courseInfo, double [] segLen){
	...
	private void getXMLTagInfoFromXML(NodeList nL){
		...
		for (int i = 0; i < nL.getLength(); i++) {
			if (nL.item(i).getNodeType() == Node.ELEMENT_NODE) {
				...
				if (el.getNodeName().equalsIgnoreCase("SEGMENT")) {
					...
					Segment segment = new Segment(Double.valueOf(segNum), Double.valueOf(segLen), Double.valueOf(segSpeedLim));
				...
				if (el.getNodeName().equalsIgnoreCase("DRIVER")) {
					...
					for (int j = 0; j < allDriverTypes.size(); j++){
						if(driverDriverType.equalsIgnoreCase(allDriverTypes.get(j).getDriverTypeName())){
							Driver driver = new Driver(driverName, time,allDriverTypes.get(j));
		...
		setXMLSegTagInfoFromXML(allSegments, segNumsL,segLensL, segSpeedLimsL);
		setXMLDriverTagInfoFromXML(allDriverTypes, allDrivers);
	}
	
	//Set the info for Segments
	private void setXMLSegTagInfoFromXML(ArrayList<Segment> allSegments, ArrayList<String>  segNumsL, ArrayList<String> segLensL, ArrayList<String>  segSpeedLimsL){
	...
	public static void main (String [] args) throws Exception {
		DriverTypesSim courseSim = new DriverTypesSim();
	}
